# -*- encoding=utf8 -*-
__author__ = "86133"

# from airtest.core.api import *
# from airtest.cli.parser import cli_setup

# if not cli_setup():
#     auto_setup(__file__, logdir=True, devices=["Windows:///1573934",])


# script content
print("start...")

__author__ = "86133"

from airtest.core.api import *

auto_setup(__file__)
# generate html report
# from airtest.report.report import simple_report
# simple_report(__file__, logpath=True)
# touch(Template(r"tpl1629568294263.png", record_pos=(0.224, -0.246), resolution=(627, 369)))
touch(Template(r"tpl1629564549076.png", record_pos=(0.224, -0.25), resolution=(627, 369)))
pos = exists(Template(r"tpl1629566720355.png", record_pos=(-0.128, 0.266), resolution=(627, 369)))
if pos:touch(pos)
touch(Template(r"tpl1629566569580.png", record_pos=(-0.325, -0.028), resolution=(627, 369)))



sleep(20)

touch(Template(r"tpl1629567592054.png", record_pos=(0.28, -0.143), resolution=(627, 369)))
touch(Template(r"tpl1629567604081.png", record_pos=(0.162, 0.114), resolution=(627, 369)))

#touch(Template(r"tpl1629567468925.png", record_pos=(-0.004, -0.224), resolution=(627, 369)))
touch(Template(r"tpl1629568294263.png", record_pos=(0.224, -0.246), resolution=(627, 369)))

pos = exists(Template(r"tpl1629566720355.png", record_pos=(-0.128, 0.266), resolution=(627, 369)))
if pos:touch(pos)
touch(Template(r"tpl1629566569580.png", record_pos=(-0.325, -0.028), resolution=(627, 369)))
sleep(20)

touch(Template(r"tpl1629567592054.png", record_pos=(0.28, -0.143), resolution=(627, 369)))
touch(Template(r"tpl1629567604081.png", record_pos=(0.162, 0.114), resolution=(627, 369)))
touch(Template(r"tpl1629568294263.png", record_pos=(0.224, -0.246), resolution=(627, 369)))

pos = exists(Template(r"tpl1629566720355.png", record_pos=(-0.128, 0.266), resolution=(627, 369)))
if pos:touch(pos)
touch(Template(r"tpl1629566569580.png", record_pos=(-0.325, -0.028), resolution=(627, 369)))
sleep(20)

touch(Template(r"tpl1629567592054.png", record_pos=(0.28, -0.143), resolution=(627, 369)))
touch(Template(r"tpl1629567604081.png", record_pos=(0.162, 0.114), resolution=(627, 369)))
touch(Template(r"tpl1629568294263.png", record_pos=(0.224, -0.246), resolution=(627, 369)))

pos = exists(Template(r"tpl1629566720355.png", record_pos=(-0.128, 0.266), resolution=(627, 369)))
if pos:touch(pos)
touch(Template(r"tpl1629566569580.png", record_pos=(-0.325, -0.028), resolution=(627, 369)))
sleep(20)

touch(Template(r"tpl1629567592054.png", record_pos=(0.28, -0.143), resolution=(627, 369)))
touch(Template(r"tpl1629567604081.png", record_pos=(0.162, 0.114), resolution=(627, 369)))
touch(Template(r"tpl1629568294263.png", record_pos=(0.224, -0.246), resolution=(627, 369)))

pos = exists(Template(r"tpl1629566720355.png", record_pos=(-0.128, 0.266), resolution=(627, 369)))
if pos:touch(pos)
touch(Template(r"tpl1629566569580.png", record_pos=(-0.325, -0.028), resolution=(627, 369)))
sleep(20)

touch(Template(r"tpl1629567592054.png", record_pos=(0.28, -0.143), resolution=(627, 369)))
touch(Template(r"tpl1629567604081.png", record_pos=(0.162, 0.114), resolution=(627, 369)))

#touch(Template(r"tpl1629567500913.png", record_pos=(0.243, 0.183), resolution=(627, 369)))
#pos1=exists(Template(r"tpl1629568389927.png", record_pos=(0.251, 0.106), resolution=(627, 369)))
# if pos1:touch(Template(r"tpl1629567500913.png", record_pos=(0.243, 0.183), resolution=(627, 369)))
# touch(Template(r"tpl1629568389927.png", record_pos=(0.251, 0.106), resolution=(627, 369)))
touch(Template(r"tpl1629568294263.png", record_pos=(0.224, -0.246), resolution=(627, 369)))
touch(Template(r"tpl1629611120335.png", record_pos=(-0.327, -0.096), resolution=(910, 530)))
touch(Template(r"tpl1629611144206.png", record_pos=(0.153, 0.129), resolution=(910, 530)))
touch(Template(r"tpl1629568389927.png", record_pos=(0.251, 0.106), resolution=(627, 369)))



